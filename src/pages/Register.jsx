...
export default function Register() {
  ...
  const handleQuickFillPhone = () => {
    setPhone('5321234567');
    showToast('CEO Modu: Telefon numarası dolduruldu!', 'success');
  };

  const handleQuickFillOtp = () => {
    const quickOtp = ['1', '9', '2', '3', '2', '3'];
    setOtp(quickOtp);
    setIsVerifying(true);
    showToast('CEO Modu: OTP Doğrulanıyor...', 'success');
    setTimeout(() => {
      setUser(prev => ({ ...prev, phone: `+905321234567`, isVerified: true }));
  ...
  return (
    <div className="screen" style={{ background: 'var(--togg-navy)' }}>
      ...
      <div style={{ padding: '0 20px 20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        {/* CEO Quick Fill Badge */}
        {!showOtp ? (
          <button 
            onClick={handleQuickFillPhone}
            style={{
              background: 'rgba(0, 212, 170, 0.1)', border: '1px solid var(--togg-teal)',
              borderRadius: '20px', padding: '6px 12px', color: 'var(--togg-teal)',
