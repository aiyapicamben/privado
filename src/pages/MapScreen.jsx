...
import { parkingZones, chargingStations, userLocation, getVehicleImage, getVehicleMarker, mockVehicles as fallbackVehicles } from '../data/mockData';
...
export default function MapScreen() {
  ...
  const socketFleet = useAdminStore((state) => state.fleetData);
  const displayVehicles = socketFleet.length > 0 ? socketFleet : fallbackVehicles;
  ...
  return (
    <div className="screen" style={{ position: 'relative', overflow: 'hidden' }}>
      ...
      <div style={{
        ...
        {/* Row 1: Unified Cockpit Brand & Action Icons */}
        <div className="animate-fadeInDown" style={{
          background: 'rgba(19, 27, 51, 0.93)',
          borderRadius: '16px',
          padding: '10px 16px',
          display: 'flex',
          alignItems: 'center',
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            <span style={{ 
              ...
              background: '#2ed573', boxShadow: '0 0 10px #2ed573, 0 0 4px #2ed573'
            }} />
            <span style={{ fontSize: '15px', fontWeight: 900, color: '#fff', letterSpacing: '1.5px', fontFamily: 'Outfit, sans-serif' }}>
              TURLA
            </span>
          </div>
          ...
          <div style={{ display: 'flex', gap: '8px' }}>
            {[
              { icon: '🛡️', title: 'Sigorta', click: () => navigateTo(APP_STATES.INSURANCE), bg: 'rgba(0,212,170,0.12)', border: 'rgba(0,212,170,0.25)', color: 'var(--togg-teal)' },
              { icon: '⚡', title: 'Şarj', click: () => showToast('En yakın şarj istasyonları filtrelendi', 'info'), bg: 'rgba(255,165,2,0.12)', border: 'rgba(255,165,2,0.25)', color: '#ffa502' },
              { icon: '💳', title: 'Cüzdan', click: () => navigateTo(APP_STATES.WALLET), bg: 'rgba(0,212,170,0.12)', border: 'rgba(0,212,170,0.25)', color: 'var(--togg-teal)' },
              { icon: '🛠️', title: 'Yönetici', click: () => navigateTo(APP_STATES.ADMIN_LOGIN), bg: 'rgba(255,71,87,0.12)', border: 'rgba(255,71,87,0.25)', color: '#ff4757' },
              { icon: '⚙️', title: 'Ayarlar', click: () => navigateTo(APP_STATES.SETTINGS), bg: 'rgba(255,255,255,0.06)', border: 'rgba(255,255,255,0.12)', color: '#fff' }
            ].map((act, idx) => (
              <button
                key={idx}
                onClick={act.click}
        ...
        <div className="animate-fadeInDown" style={{
          ...
          <button
            ...
            style={{
              background: showLegend ? 'rgba(0,212,170,0.15)' : 'rgba(255,255,255,0.04)',
              border: '1px solid rgba(255,255,255,0.08)',
              borderRadius: '9px',
              width: '32px',
              height: '32px',
      ...
      <div className="animate-fadeInUp" style={{
        ...
        <div style={{
          ...
          <span style={{ fontSize: '12px', fontWeight: 600, color: 'var(--togg-gray-300)' }}>
            {displayVehicles.length} araç yakınında
          </span>
        </div>
      </div>
